...
| [workflows/gated-distribution](workflows/gated-distribution/) | A distribution workflow that holds the tested AMI for release approval - WaitForAction in the distribution stage, then a region copy, an optional launch-permission grant, and SSM parameters, chained through per-region step outputs |
| [workflows/packer-build](workflows/packer-build/) | An unchanged HashiCorp Packer template as the pipeline's build stage - a custom build workflow runs Packer on the build instance, hands the AMI back through the workflow's ImageId output, and the managed test stage, distribution, and lifecycle operate on the Packer-built AMI |
